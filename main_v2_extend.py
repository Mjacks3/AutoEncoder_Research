...
def train(args):
    ...
    for epochs_this_iteration in range(0,500,100):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--update-interval', default=100, type=int,
...
if args.experiment:
    ...
    #pretrained_weights
    #Training
    models = [10,19,2,18,4,17,11,3,6,14]
    for num in models:
        args.num_clusters = int(num)
        args.pretrained_weights = "experiment/"+str(num)+"/ae_weights.h5"
        print(args.pretrained_weights)
        
        args.save_dir = "experiment/round_3/" + str(args.num_clusters)
    ...
    for num_clusters in models:
        ...
        for sub_epoch_count in range(100,500,100):

            for r, d, f in os.walk("experiment/case_study"): 

                if len(f) > 1 and r.split("/")[-1] +  "_" + str(args.num_clusters)+ "-"+ str(sub_epoch_count)+ ".clustering" not in f  :
                    if os.path.exists("experiment/round_3/"+str(num_clusters)+"/"+str(sub_epoch_count)+"/model_final.h5"):
                        
                        args.weights = "experiment/round_3/"+str(num_clusters)+"/"+str(sub_epoch_count)+"/model_final.h5"
                        args.test_dataset = r
                        test(args)
                else:
                    print(r.split("/")[-1] +  "_" + str(args.num_clusters)+ "-"+ str(sub_epoch_count)+ ".clustering")
